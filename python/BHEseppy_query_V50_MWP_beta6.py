...
def main():
 ...
 ###DEFINE FIXED PARAMETERS (FOR EXPERT USE)
    ...
    gradient = float("0.03")# ground temperature gradient (K/m) for depth dependent ground temperature
 ...
 ### DEFINE BHE Field
   ...
   # if the field is defined by retangular array N_1 x N_2
    ...
    if (nBoreholes > 0):
        PHZ_L3, PKL_L3, Efactor, SCOP, SEER, image_hash, image_hash_borefield = calculateL3(GTcalc, lamda, BS_HZ_L3, BS_KL_L3, Tmin, Tmax, years, D, H, r_b, cv, Pfactor, flowrate, boreField, PNGPath, PNGPath_borefield, T_radiator, DEBUG, RESULT)
    ...
    if (nBoreholes > 0 and BALANCED > 0):
        ...
        PHZ_L3_bal, PKL_L3_bal, Efactor_bal, SCOP_bal, SEER_bal, image_hash_bal, _ = calculateL3(GTcalc, lamda, BS_HZ_bal, BS_KL_bal, Tmin, Tmax, years, D, H, r_b, cv, Pfactor_bal, flowrate, boreField, PNGPath_bal, "", T_radiator, DEBUG, RESULT)       

    if (RESULT): printdemand(case, P_HZ, P_KL, BS_HZ, BS_KL, COP, EER, T_radiator, GTcalc, lamda)
    if (RESULT): printratio(Efactor, Pfactor) 
    ...
    if (COP > 0):  Pel_heatpump_user =   P_HZ_user / (COP - 1)   # electric power for heating kW
    if (EER > 0):  Pel_chiller_user  =   P_KL_user / (EER + 1)   # electric power for cooling kW
    if (SCOP > 0): Eel_heatpump_user =   E_HZ_user / (SCOP - 1)    # electric power for heating kWh
    if (SEER > 0): Eel_chiller_user  =   E_KL_user / (SEER + 1)    # electric power for cooling kWh
    if (P_HZ > 0):    coverHZ = 100.0 / (P_HZ*BS_HZ) * E_HZ_user    # %
    if (P_KL < 0):    coverKL = 100.0 / (P_KL*BS_KL) * E_KL_user    # %
    if (np.isfinite(coverHZ) or np.isfinite(coverKL)): cover_user    =   np.nanmin([coverHZ, coverKL])   # %
    
    if (RESULT): printresults(case, P_HZ_user, P_KL_user, E_HZ_user , E_KL_user, cover_user, Pel_heatpump_user, Pel_chiller_user, Eel_heatpump_user, Eel_chiller_user, nBoreholes, H, BB_L3, BB_mean, 0., COP, EER, SCOP, SEER)
    if (RESULT): printratio(Efactor_user, Pfactor)
    user_defined_results = [case, P_HZ_user, P_KL_user, E_HZ_user , E_KL_user, cover_user, Pel_heatpump_user, Pel_chiller_user, Eel_heatpump_user, Eel_chiller_user, nBoreholes, H, BB_L3, BB_mean, 0., COP, EER, SCOP, SEER, Efactor_user, image_hash, image_hash_borefield, GTcalc]
    ...
    if (BALANCED > 0):
        ...
        if (COP > 0): Pel_heatpump_bal =   P_HZ_bal / (COP - 1)     # electric power for heating kW
        if (EER > 0): Pel_chiller_bal  =   P_KL_bal / (EER + 1)     # electric power for cooling kW
        if (SCOP_bal > 0): Eel_heatpump_bal =   E_HZ_bal / (SCOP_bal - 1)     # electric power for heating kW
        if (SEER_bal > 0): Eel_chiller_bal  =   E_KL_bal / (SEER_bal + 1)     # electric power for heating kW
        if (P_HZ > 0):    coverHZ = 100.0 / (P_HZ*BS_HZ) * E_HZ_bal     # %
        if (P_KL < 0):    coverKL = 100.0 / (P_KL*BS_KL) * E_KL_bal     # %
        if (np.isfinite(coverHZ) or np.isfinite(coverKL)): cover_bal    =   np.nanmin([coverHZ, coverKL])   # %
        if (Efactor_user < 1): #more cooling
            cover_rise = (P_KL_bal-P_KL_user) / P_KL_user * 100.
        else:                   #more cooling
            cover_rise = (P_HZ_bal-P_HZ_user) / P_HZ_user * 100.
        
        if (RESULT): printresults("AUTO-BALANCED", P_HZ_bal, P_KL_bal, E_HZ_bal, E_KL_bal, cover_bal, Pel_heatpump_bal, Pel_chiller_bal, Eel_heatpump_bal, Eel_chiller_bal, nBoreholes, H, BB_L3, BB_mean, cover_rise, COP, EER, SCOP_bal, SEER_bal)
        if (RESULT): printratio(Efactor_bal, Pfactor_bal)
    
    automatic_results = [BALANCED, P_HZ_bal, P_KL_bal, E_HZ_bal, E_KL_bal, cover_bal, Pel_heatpump_bal, Pel_chiller_bal, Eel_heatpump_bal, Eel_chiller_bal, nBoreholes, H, BB_L3, BB_mean, cover_rise, COP, EER, SCOP_bal, SEER_bal, Efactor_bal, image_hash_bal, BS_HZ_bal, BS_KL_bal, T_radiator]
...
def printdemand(text, PHZ, PKL, BS_HZ, BS_KL, COP, EER, T_radiator, GTcalc, lamda):
    ...
    if (COP > 0):print(f"    PHZ   = {BS_HZ:.1f} h x {PHZ/(1-1/COP):.1f} kW")
    else:        print(f"    PHZ   = {BS_HZ:.1f} h x {PHZ:.1f} kW")
    if (EER > 0):print(f"    PKL   = {BS_KL:.1f} h x {PKL/(1+1/EER):.1f} kW")
    else:        print(f"    PKL   = {BS_KL:.1f} h x {PKL:.1f} kW")
    print(f"    T_Vorlauf_Heizung: {T_radiator:.1f} °C")
    print(f"Vorgabe für Erdsonden:")
    if (COP > 0): print(f"    COP  = {COP:.1f}")
    if (EER > 0): print(f"    EER  = {EER:.1f}")
...
def printratio(Efactor, Pfactor):
    if (Efactor>1/1000. and Efactor<1000.): print(f"    energy ratio HZ/KL = {Efactor:.3f}")
    if (Pfactor>1/1000. and Pfactor<1000.): print(f"    power  ratio HZ/KL = {Pfactor:.3f}")
...
def printresults(text, PHZ, PKL, EHZ, EKL, cover, P_heatpump, P_chiller, E_heatpump, E_chiller, nBoreholes, H, BB_L3, BB_mean, cover_rise, COP, EER, SCOP, SEER):
    if (np.isfinite(cover)):  covert=(f"{cover:.1f} %")
    else:                     covert=("not defined")
    
    print(f"\nRESULTS {text} for {nBoreholes} BHEs with {H} m depth and average spacing {BB_mean} {BB_L3}:")
    print(f"    heat extraction power from BHE                   = {PHZ:.1f} kW")
    print(f"    + electr. power of heat pump (COP = {COP:.1f})         = {P_heatpump:.1f} kW")
    print(f"    = heating power for building                     = {PHZ+P_heatpump:.1f} kW")
    print(f"    yearly heat extraction from BHE                  = {EHZ/1000.:.1f} MWh/a")
    print(f"    + electr. energy of heat pump (SCOP = {SCOP:.1f})       = {E_heatpump/1000.:.1f} MWh/a")
    print(f"    = yearly heating energy for building             = {(EHZ+E_heatpump)/1000.:.1f} MWh/a")
    print("\n")
    print(f"    heat injection power to BHE                      = {PKL:.1f} kW")
    print(f"    - electr. power of heat pump (EER = {EER:.1f})         = {P_chiller:.1f} kW")
    print(f"    = cooling power for building                     = {PKL-P_chiller:.1f} kW")
    print(f"    yearly heat injection to BHE                     = {EKL/1000.:.1f} MWh/a")
    print(f"    - electr. energy of heat pump (SEER = {SEER:.1f})       = {E_chiller/1000.:.1f} MWh/a")
    print(f"    = yearly cooling energy for building             = {(EKL-E_chiller)/1000.:.1f} MWh/a")
    print("\n")
    print(f"    Coverage from BHEs                               = {covert}")
    if (cover_rise > 0): print(f"    {cover_rise:.1f} % more energy use @ balanced mode")
...
def calculateL3(T_g, lamda, BS_HZ, BS_KL, Tmin, Tmax, years, D, H, r_b, cv, Pfactor, m_flow_borehole, boreField, PNGPath,pngname2, T_radiator, DEBUG, RESULT):
    ...
    elif (BS_KL > 0 and Pfactor < 1000.):  Efactor = Pfactor * BS_HZ / BS_KL
    ...
    if (round(Efactor,2) < 1):              #more cooling than heating: Beginn with cooling period
        ...
        if (abs(PHZ2)<abs(PHZ)):
            log(f" Heizlimit im Jahr 2 überschritten, reduziere Heizleistung um Faktor: {PHZ2/PHZ:.3f} !")
        ...
        if (abs(PKL2)<abs(PKL)):
            log(f" Kühllimit im Jahr 2 überschritten, reduziere Heizleistung um Faktor: {PKL2/PKL:.3f} !")
            log(f" Pot PHZ precalc   = {PHZ:6.2f} W/m")
            PKL=PKL2
            PHZ=-PKL2*Pfactor
    ...
    if (np.any(np.isfinite(COP_h))): SCOP = np.nanmean(COP_h)
    if (np.any(np.isfinite(EER_h))): SEER = np.nanmean(EER_h)
...
def plotborefield(boreField,pngname, RESULT):
    ...
    if (RESULT): print(f"create plot {pngname}")
    fig = gt.boreholes.visualize_field(boreField, viewTop=True, view3D=False, labels=True)#, showTilt=True)
...
def get_COP_potential (T_sink_high, T_Source_low, P_SOW35):
    ...
    Qc = hpfunc (Qc_a_10K, Qc_b_10K, Qc_c_10K, T_Source_low, T_sink_high) * P_SOW35 / 44.14
    Qe = hpfunc (Qe_a_10K, Qe_b_10K, Qe_c_10K, T_Source_low, T_sink_high) * P_SOW35 / 44.14
    Pel = hpfunc (Pel_a_10K, Pel_b_10K, Pel_c_10K, T_Source_low, T_sink_high) * P_SOW35 / 44.14
    #COP = Qc / Pel
    #EER = Qe / Pel
